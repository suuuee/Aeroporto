...
var areas = [
	{ "label": "Immigration Area", "startRow": serverRow - 1, "numRows": 6, "startCol": serverCol, "numCols": 6, "color": "blue" },
	{ "label": "Seating Area", "startRow": 17, "numRows": 2, "startCol": 8, "numCols": 6 },
	{ "label": "Transit Area", "startRow": 5, "numRows": 20, "startCol": 3, "numCols": 30, "color": "red" },
...
var seatingCoordinates = [
	...
	[seatingArea.startRow + 1, seatingArea.startCol + 1],
	[seatingArea.startRow + 1, seatingArea.startCol + 2],
	[seatingArea.startRow + 1, seatingArea.startCol + 3],
	[seatingArea.startRow + 1, seatingArea.startCol + 4],
	[seatingArea.startRow + 1, seatingArea.startCol + 5]
];

// Each shop is 3x3
var shopCoordinates = [
	// first row, first shop
	[9, 16],
	[9 + 1, 16],
	[9 + 2, 16],
	[9, 16 + 1],
...
function addDynamicAgents() {
	...
	if (Math.random() < probArrival) {
		var newagent = {
			"id": 1, "location": { "row": serverRow, "col": 39 },
			"target": {
				"row": serverRow,
				"col": serverCol + 2 + queue
			},
			"state": UNCHECKED,
...
function updateagent(agentIndex) {
	...
	switch (state) {
		case UNCHECKED:
			...
			break;
		case WAITING:
			if (server.state == IDLE & hasArrived) {
				if (agent.id == nextTreatedagentID) {
					agent.target.row = serverRow;
					agent.target.col = serverCol + 1;
					server.state = BUSY;
